...
import { decisionOptions, presentDecision } from "@/lib/agentic/presentation/decision";
...
export async function evidenceTool(input: Pick<AgenticRuntime, "config" | "now" | "scope" | "store"> & Readonly<{
  planHandle: string; expectedRevision: number; ingredientId?: string; productId?: string;
}>) {
  ...
  const option = decisionOptions(saved.result)[0];
  if (!option) return businessError({ reasonCode: "not_found", fieldPath: "planHandle", message: "Not found." });
  const decision = presentDecision(saved.result, input.planHandle, input.expectedRevision);
  const choice = "choices" in decision ? decision.choices[0] : undefined;
  ...
  return { ok: true as const, planHandle: input.planHandle, revision: input.expectedRevision,
