...
import { canonicalPublicToolName, type JsonRpcRequest } from "@/lib/agentic/mcp/rpc";
...
export function requestSetupRecovery(error: unknown, body: unknown, correlationId: string) {
  ...
  const params = call.params as { name?: unknown; arguments?: { operation?: unknown } } | undefined;
  if (call.method !== "tools/call" || typeof params?.name !== "string") return null;
  const poll = canonicalPublicToolName(params.name) === "plan" && params.arguments?.operation === "get";
  return businessError({ reasonCode: "temporarily_unavailable", correlationId,
    message: poll ? "Plan status is temporarily unavailable. Poll the same plan handle; do not start another match." : "The service is temporarily unavailable. Retry the same request with its original idempotency key and unchanged input.",
