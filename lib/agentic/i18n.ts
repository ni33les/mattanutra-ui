...
const MESSAGES: Record<string, Record<(typeof AGENTIC_LOCALES)[number], string>> = {
  "plan.summary.refresh_required": {
    en: "Refresh this saved plan against the current catalogue and contract before selecting or creating checkout. Send scoring:{} and the current revision; your health information is preserved.",
    th: "อัปเดตแผนที่บันทึกไว้ตามแคตตาล็อกและข้อกำหนดปัจจุบันก่อนเลือกหรือชำระเงิน ส่ง scoring:{} และ revision ปัจจุบัน โดยข้อมูลสุขภาพของคุณจะยังคงอยู่",
    "zh-CN": "选择或创建结账前，请根据当前目录和协议刷新已保存的方案。使用scoring:{} 和当前 revision；您的健康信息会保留。"
  },
  ...
  "plan.summary.simpler_partial": { "en": "A {count}-product option ({products}) covers {covered}, with these remaining gaps: {gaps}. This is an optional trade-off, not a complete substitute.", "th": "ทางเลือก {count} ผลิตภัณฑ์ ({products}) ครอบคลุม {covered} แต่ยังขาด: {gaps} เป็นทางเลือกที่ต้องแลกเปลี่ยนความครอบคลุม ไม่ใช่การทดแทนที่ครบถ้วน", "zh-CN": "另有含 {count} 款产品的选项（{products}），覆盖 {covered}，但仍有缺口：{gaps}。这是可选的取舍，并非完整替代。" },
  "guidance.scope_selected": { "en": "Selected: {message}", "th": "ตัวเลือกที่เลือก: {message}", "zh-CN": "已选方案：{message}" },
  "guidance.scope_alternative": { "en": "Highlighted alternative: {message}", "th": "ทางเลือกที่แนะนำ: {message}", "zh-CN": "推荐备选方案：{message}" },
  "guidance.scope_both": { "en": "Both options: {message}", "th": "ทั้งสองตัวเลือก: {message}", "zh-CN": "两个方案：{message}" },
  "guidance.scope_plan": { "en": "Plan-wide: {message}", "th": "คำแนะนำสำหรับแผนทั้งหมด: {message}", "zh-CN": "整个计划：{message}" },
  "guidance.measured_finding": { "en": "{nutrient}: {exposure} {unit}; reference {reference} {unit}.", "th": "{nutrient}: {exposure} {unit}; ค่าอ้างอิง {reference} {unit}", "zh-CN": "{nutrient}：{exposure} {unit}；参考值 {reference} {unit}。" },
  ...
  "plan.matching.empty_practical_fit": {
    en: "No new products have the lowest overall penalty among evaluated choices under these dose, routine and price preferences. Some targets remain unmet. A returned purchase option may improve coverage with the disclosed trade-offs; review it if you want to buy.",
    th: "ภายใต้ความต้องการด้านปริมาณ ความสะดวก และราคานี้ การไม่เพิ่มสินค้ามีคะแนนโทษรวมต่ำที่สุดในตัวเลือกที่ประเมิน แต่บางเป้าหมายยังไม่ครบ ตัวเลือกที่ซื้อได้อาจเพิ่มความครอบคลุมพร้อมข้อแลกเปลี่ยนที่ระบุ โปรดพิจารณาหากต้องการซื้อ",
    "zh-CN": "按这些剂量、日常用量和价格偏好，在已评估的选项中，不新增商品的综合惩罚最低，但部分目标仍未满足。返回的可购买选项可能提高覆盖率，并附有明确的取舍；如需购买，可查看这些选项。"
  },
