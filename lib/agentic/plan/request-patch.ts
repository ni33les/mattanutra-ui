...
export function mergeRequestPatch(base: PlanRequest, patch: PlanRequestPatch): PlanRequest | AgenticErrorResult {
  ...
  let merged: unknown;
  try { merged = merge(base, patch); } catch { return businessError({ fieldPath: "requestPatch", reasonCode: "invalid_request", message: "Only product, pill and price preferences can be cleared with null; other fields cannot be null or use unsafe property names. Omit a field to preserve it or use [] to clear an array." }); }
...
export function originalRequestFor(result: PlanResult): PlanRequest | AgenticErrorResult {
  ...
  const requirements = !result.contractVersion || Number(result.contractVersion.split(".")[0]) < 5
  ...
  return {
    ...
    profile: state.profile, requirements,
