...
export function formatNutrientAmount(amount: number, unit?: string | null, requestedAmount?: number | null) {
  ...
  const smallAmountPrecision = amount !== 0 && Math.abs(amount) < (unit === "IU" ? 0.5 : 0.01) ? Math.ceil(-Math.log10(Math.abs(amount))) + 1 : 0;
  const decimals = unit === "IU" && Number.isInteger(amount) ? 0 : Math.min(20, Math.max(unit === "IU" ? 0 : 2, unit === "IU" ? 0 : requestedDecimals, smallAmountPrecision));
