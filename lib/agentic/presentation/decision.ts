...
const COPY = {
  en: { processing: "Updating your plan. Wait before checking again.", failed: "Matching did not finish. Retry this refinement with scoring:{} and a new idempotency key.",
    ready: "Your recommended routine is ready. Proceed to checkout or adjust the weights.",
  ...
  th: { processing: "กำลังอัปเดตแผน โปรดรอก่อนตรวจสอบอีกครั้ง", failed: "การจับคู่ยังไม่เสร็จ ลองส่ง scoring:{} พร้อมคีย์ idempotency ใหม่",
    ready: "ชุดที่แนะนำพร้อมแล้ว ไปชำระเงินหรือปรับน้ำหนักความสำคัญได้",
...
export function presentDecision(result: PlanResult, planHandle: string, revision: number): SimplePlanDecision {
  ...
  const options = decisionOptions(result);
