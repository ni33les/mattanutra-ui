...
export const READY_MEANING = "Ready means technically selectable/checkout-ready, not complete coverage or medical approval.";
...
profile means customer context. Start with scoring.profile=best_match (the default), then adjust weights from the conversation. ${importanceInstructions()} Up to six decimal places. Omission preserves; individual null resets; weights:null clears; preset changes reset overrides. A weight is not a dose or categorical exclusion.
