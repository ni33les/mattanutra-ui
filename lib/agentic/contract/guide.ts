...
export const CLIENT_EXAMPLES = [
  ...
  { name: "partial-request", tool: "plan", arguments: { idempotencyKey: "example-partial-key-0001", locale: "en", destinationCountry: "TH", targets: [{ name: "D3", amount: 1000, unit: "IU" }, { name: "Unlisted example ingredient", amount: 50, unit: "mg" }] } },
...
export function clientDiscovery(localeInput?: string, view: "overview" | "client_guide" | "plan_schema" = "overview", environment: AgenticEnvironment = "dev") {
  ...
  return { clientInstructions: `${positioning(locale).invocationGuidance} ${environmentWarning(environment, locale)}\n${OVERVIEW_CARD}${locale === "en" ? "" : `\n${importanceInstructions(locale)}`}`,
    clientExamples: (view === "overview" ? CLIENT_EXAMPLES.filter(example => ["create-provisional-targets", "read-or-poll"].includes(example.name)) : CLIENT_EXAMPLES).map(example => ({ ...example, arguments: { ...structuredClone(example.arguments), ...("locale" in example.arguments ? { locale } : {}) } })) };
}
