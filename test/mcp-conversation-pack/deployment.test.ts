...
test("CONVERSATION-DEPLOY-01 scoped release proof rejects missing, changed and wrong-environment evidence", () => {
  const root = mkdtempSync(join(tmpdir(), "conversation-proof-"));
  ...
  const proof = { version: "dev-mcp-conversation-1", environment: "dev", scope: "mcp_conversation_payload", contractVersion: "7.2.4", ...expected,
  ...
  assert.doesNotThrow(() => checkMcp721Proof(path, expected, "conversation"));
  for (const patch of [{ environment: "uat" }, { passed: false }, { sourceCommit: "stale" }, { stages: proof.stages.slice(1) }, { artifacts: proof.artifacts.slice(1) }]) {
    save("attestation.json", { ...proof, ...patch }); assert.throws(() => checkMcp721Proof(path, expected, "conversation"));
  }
  save("attestation.json", proof); save("tests.json", { passed: false }); assert.throws(() => checkMcp721Proof(path, expected, "conversation"));
});
