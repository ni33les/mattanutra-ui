...
test("M722-DEPLOY-01 scoped release proof rejects missing, changed and wrong-environment evidence", () => {
  ...
  const proof = { version: "dev-mcp-722-1", environment: "dev", scope: "mcp_722_open_points", contractVersion: "7.2.2", ...expected,
  ...
  assert.doesNotThrow(() => checkMcp721Proof(path, expected, "722"));
  for (const patch of [{ environment: "uat" }, { passed: false }, { sourceCommit: "stale" }, { stages: proof.stages.slice(1) }, { artifacts: proof.artifacts.slice(1) }]) {
    save("attestation.json", { ...proof, ...patch }); assert.throws(() => checkMcp721Proof(path, expected, "722"));
  }
  save("attestation.json", proof); save("tests.json", { passed: false }); assert.throws(() => checkMcp721Proof(path, expected, "722"));
});
