...
import { installRealCatalogue, uninstallRealCatalogue, runtime, profile, rpc } from "../ax-refinement/helpers.ts";
...
test("test_expanded_single_job_reaches_terminal_state", { timeout: 185000 }, async () => {
  ...
  while (completed.status === "processing" && performance.now() - start < 180000) {
    await delay(1000);
    completed = await rpc(app, "plan", { operation: "get", planHandle: initial.planHandle });
  }
  elapsedMs = performance.now() - start;
  assert.ok(elapsedMs <= 180000); assert.notEqual(completed.status, "processing");
  assert.equal(completed.ok, true, JSON.stringify(completed));
  const search = completed.searchSummary as { expansionAttempts: number; expansionBudget: number };
...
test("test_expanded_does_not_relax_constraints", async () => {
  ...
  const full = await rpc(app, "plan", { operation: "get", planHandle: completed.planHandle, responseView: "full" });
  assert.equal(full.ok, true); assert.ok((full.options as unknown[]).length > 0);
  const options = full.options as { basket: { productId: string }[] }[];
  assert.ok(options.some(option => option.basket.length));
  for (const option of options) for (const item of option.basket) assert.ok(!request.requirements.excludeProductIds!.includes(item.productId));
  ...
  assert.deepEqual(saved.targets, request.targets);
});
