...
test("test_conversation_advice_capped_to_selected_and_highlighted", () => {
  ...
  assert.deepEqual(result.advice.flatMap(row => row.guidanceIds), ["selected", "highlight"]); assert.ok(result.advice.length <= 5);
  ...
  assert.ok(selected.advice.some(row => row.guidanceIds.includes("background")));
});
test("test_incomplete_information_is_plan_level_once", () => {
  ...
  assert.deepEqual(rows[0].guidanceIds, ["missing-0", "missing-1"]);
  const details = projectPlan(plan, { responseView: "details", expectedRevision: plan.revision, sections: ["advice"] });
  assert.ok(details.ok); assert.deepEqual(details.options.map(row => row.advice), plan.options!.map(row => row.advice));
});
test("test_dose_review_without_threshold_is_not_high_severity", () => {
  ...
  const result = projectPlan(plan, { responseView: "conversation" });
  const missing = result.advice.find(row => row.kind === "incomplete_information")!; assert.ok(missing);
  assert.equal(missing.kind, "incomplete_information"); assert.equal(missing.severity, "info");
  assert.ok(result.advice.some(row => row.kind === "dose_review" && row.severity === "high"));
});
test("test_no_float_junk_in_messages", () => {
  ...
  assert.match(message, /0\.3 exceeds/); assert.doesNotMatch(message, /30000000000000004/);
  assert.equal(plan.options![0].advice![0].exposure, 0.1 + 0.2, "Formatting must not change numerical evidence");
});
