...
test("test_hosted_plan_get_schema_advertises_response_view", () => {
  if (installed) {
...
test("test_hosted_plan_create_schema_defaults_conversation", () => {
  if (installed) {
...
test("test_hosted_plan_description_advertises_service", () => {
  const description = installed ? declaration() : plan.description;
  assert.match(description, /Thailand/); assert.match(description, /finite/i);
