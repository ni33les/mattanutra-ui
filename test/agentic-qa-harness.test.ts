...
import { getEvidenceBundle, getScenarioRun, startScenarioRun } from "../lib/agentic/qa/control-plane.ts";
...
describe("DEV internal QA harness", () => {
    ...
    it("proves decline then same-order success with stable evidence", async () => {
        ...
        assert.deepEqual(paidView.assertions.filter((item) => item.result === "fail"), []);
        ...
        if ("assertions" in replayView) {
            assert.deepEqual(replayView.assertions.filter((item) => item.result === "fail"), []);
        }
    ...
    it("returns redacted D7/D8 counts from opaque orderHandle only", async () => {
        ...
        if (runtime.config.environment === "dev") {
            assert.equal(typeof (info as {
                latency?: {
                    info?: {
                        p95Ms?: number;
                    };
                };
            }).latency?.info?.p95Ms, "number");
            assert.equal(typeof (infoAgain as {
                latency?: {
                    execute?: {
                        p95Ms?: number;
                        p99Ms?: number;
            ...
            assert.equal(typeof (infoAgain as {
                ...
                ?.p99Ms, "number");
            assert.equal((infoAgain as {
                latency?: {
                    buildId?: string;
                };
            }).latency?.buildId, runtime.config.buildId);
        }
    });
