...
const create = (idempotencyKey: string, input = request) => ({ idempotencyKey, ...input });
function deferred() {
    let resolve!: () => void;
...
async function call(runtime: AgenticRuntime, args: Record<string, unknown>, complete = true) {
    const response = await (complete ? handleCompletedJsonRpc : handleJsonRpc)(runtime, { id: 1, method: "tools/call", params: { name: "plan", arguments: args } });
...
async function operation(runtime: AgenticRuntime, key: string) {
    ...
    assert.ok(row, "Admitted work must have a durable operation");
    return row;
}
...
describe("MCP interrupted plan recovery", { timeout: 15000 }, () => {
        ...
        it(`recovers targets and current supplements supplied by ${explicitIds ? "ID" : "name"}`, async () => {
            ...
            const input = { ...request,
                targets: request.targets.map(target => ({ ...target,
                    ...(explicitIds ? { ingredientId: supplements.find(item => item.name === target.name)!.supplementId } : {}) })),
            ...
            assert.equal((pending.result as PlanResult).pendingInput?.request.targets[0]?.amount, input.targets[0]!.amount);
    ...
    it("recovers a current processing row from its durable original input", async () => {
        const runtime = runtimeFor();
        ...
        const receipt = (await runtime.store.getIdempotency("plan", owner, args.idempotencyKey))!;
        const { planHandle } = JSON.parse(receipt.responseJson);
        const poll = await call(runtime, { planHandle });
        assert.equal(poll.status, "processing");
        assert.equal((await operation(runtime, args.idempotencyKey)).command.payload.request!.targets[0]!.amount, request.targets[0]!.amount, "Durable input survives presentation omissions");
        const changed = await call(runtime, create(args.idempotencyKey, { ...request, targets: [{ ...request.targets[0]!, ingredientId: fixtureSnapshot().supplements.find(row => row.name === "Vitamin D3")!.supplementId, amount: 2000 }] }));
    ...
    it("uses the replacement input when explicitly revising an interrupted plan", async () => {
        ...
        const changed = await call(runtime, {
            planHandle, expectedRevision: 1, idempotencyKey: "recovery-replacement-input",
            ...{ ...request, targets: [{ ...request.targets[0]!, ingredientId: fixtureSnapshot().supplements.find(row => row.name === "Vitamin D3")!.supplementId, amount: 2000 }] }
        });
    ...
    it("does not reinterpret an explicit unknown ID as a valid name on resume", async () => {
        ...
        const args = create("recovery-invalid-id", { ...request,
            targets: [{ ...request.targets[0]!, ingredientId: "sup_eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee" }] } as typeof request);
        assert.equal((await call(runtime, args)).error?.reasonCode, "incompatible_identity");
        assert.deepEqual(await runtime.store.listPlanIdsByPrincipal(runtime.scope.principalScope!), [], "Unknown IDs are rejected before admission");
    });
    it("a conflicting follower cannot evict the original active request", async () => {
        ...
        let matchEntries = 0;
        setMatcherGateForTests(gate.promise);
        setMatcherEnteredForTests(() => { matchEntries++; entered.resolve(); });
        ...
        try {
            ...
            assert.equal((await call(runtime, { planHandle: admitted.planHandle }, false)).status, "processing");
            gate.resolve();
            await worker;
            const left = await call(runtime, args), right = await call(runtime, args);
            assert.equal(left.status, "no_purchase");
    ...
    it("late cancelled work cannot remove or overwrite its replacement", async () => {
        ...
        try {
            ...
            setMatcherGateForTests(null);
            setMatcherEnteredForTests(null);
            const replacement = await call(runtime, { planHandle: admitted.planHandle, expectedRevision: 1,
                idempotencyKey: "recovery-replaced-cancelled", ...{ ...request, targets: [{ ...request.targets[0], ingredientId: fixtureSnapshot().supplements.find(row => row.name === "Vitamin D3")!.supplementId, amount: 2000 }] } });
