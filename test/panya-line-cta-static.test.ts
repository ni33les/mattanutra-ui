...
describe("reveal Panya LINE CTA", () => {
  it("places Panya as a top-level reveal section instead of a checkout widget", async () => {
    ...
    assert.match(inlineQr, /className="flow-root mn-font-body text-sm leading-6 text-\[var\(--mn-ink-soft\)\]"/);
    ...
    assert.match(inlineQr, /sm:float-right/);
    assert.match(inlineQr, /clear-both mt-4/);
    assert.doesNotMatch(inlineQr, /font-serif/);
    assert.doesNotMatch(inlineQr, /uppercase tracking-\[0\.16em\]/);
    assert.doesNotMatch(inlineQr, /font-mono/);
