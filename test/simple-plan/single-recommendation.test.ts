...
test('MCP-SINGLE-01 each round exposes only the current recommendation, preserving full facts and internal candidates', () => {
  ...
  assert.notDeepEqual(refined.choices[0].products, response.choices[0].products);
});
...
test('MCP-SINGLE-02 empty recommendations expose gaps and weight refinement, never a fallback menu', () => {
  ...
  for (const locale of ['en', 'th', 'zh-CN']) {
    ...
    assert.ok(!("recommendedCandidateKey" in response)); assert.equal(response.nextAction, 'change_request');
...
test('MCP-SINGLE-03 hidden alternatives are not available through selection or internal fact lookup', async () => {
  ...
  const hidden = result.alternatives[0], candidateKey = hidden.candidateKey;
  for (const [name, fields] of [
    ['plan', { selectedCandidateKey: candidateKey, idempotencyKey: 'single-hidden-selection' }],
    ['evidence', { productId: hidden.basket[0].productId }]
  ] as const) {
    ...
    assert.equal(body.ok, false); assert.equal(body.error?.reasonCode, name === 'plan' ? 'invalid_request' : 'not_found');
  }
});
