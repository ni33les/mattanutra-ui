...
test("M721-HOST-01 current six-tool exports pass while missing retained tools and native-only evidence fail", () => {
  ...
  const missing = installed(); missing.tools = missing.tools.filter(row => row.name !== "feedback");
...
test("M721-HOST-03 connector artifacts advertise six tools, host names and the same honest service scope", () => {
  for (const provider of ["openai", "anthropic", "xai"]) {
    const adapter = JSON.parse(readFileSync(`lib/agentic/adapters/${provider}.json`, "utf8"));
    assert.doesNotMatch(adapter.instructions, /Tools:.*\bevidence\b/); assert.match(adapter.instructions, /host/i);
