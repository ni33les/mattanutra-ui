...
test('SPLAN-STATE-04 previously confirmed routine is the single decision even with an internal empty candidate', async () => {
  ...
  const empty={...result.selected!,candidateKey:'empty_best_fit',roles:['best_match' as const],basket:[]};
  const decision=presentDecision({...result,status:'ready',selected:chosen,alternatives:[empty],requestSnapshot:{...result.requestSnapshot,pinnedCandidateKey:chosen.candidateKey}},'cap_current_fallback_selection',2);
  assert.equal(decision.status,'ready');assert.equal(decision.nextAction,'execute');assert.ok(!('selectedCandidateKey' in decision));assert.ok('choices' in decision);assert.equal(decision.choices.length,1);assert.deepEqual(decision.choices[0].products.map(row=>row.productId),chosen.basket.map(row=>row.productId));
});
