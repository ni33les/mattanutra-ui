...
## 2026-09-10 — MCP recommended-limit advice only

User-approved change: public MCP ingredient advice and explicit evidence findings contain only quantified exposure strictly above a positive MattaNutra reference limit. Replace unsolicited interaction, overlap, missing-reference, continued-dose and label warnings (including the summary's unassessed-context paragraph). Do not imply medical clearance. Preserve numerical facts, unknowns, sources, internal safety records, web warnings, scoring, eligibility and checkout.

SPLAN-ADV-01/03, SPLAN-ADV-04, AE-06/07/08, AE-09 and DISC-TRUTH-03 now assert the narrower public projection. The CKD checkout cases preserve internal context and review records; their public assertions require only true limit excesses. MCP-LIMIT-01–05 add boundary, uncertainty, localized-message, evidence and publication regressions. Identical excesses with different sources are shown once per choice; narrow evidence retains the individual sources. All historical evidence and prices remain unchanged.

## MCP single recommendation (2026-09-10)

- User correction: return one recommendation per conversational round, never a simultaneous menu. Existing `choices` is bounded to at most one; its current ID still supports revision-safe confirmation and narrow evidence. Internal matcher alternatives, prices, arithmetic and web behavior are unchanged. Hidden internal IDs cannot be selected or queried.
- `best_match` is the public default name for the existing balanced coefficients; `balanced` remains an input alias. Sparse weights persist and reset through the existing protocol. No new response fields, tools, locks or matching passes.
- `MCP-SINGLE-01–05` failed against cf9aa574 before implementation (RED commit 73798c65). Coverage includes retained internal candidates, empty recommendations, hidden IDs, default weights and aligned publication.
- AX5-03/04 and AX6-01/05 retain original prices, doses, pills and arithmetic across successive single-result rounds instead of three baskets in one payload. V5-CLIENT-03 confirms only the current returned recommendation. The localized empty-recommendation cases now refine before confirmation instead of selecting an unreturned fallback. Original case IDs and fixture prices remain preserved.
- No-purchase gaps use `change_request` rather than `review_options`. Already covered targets still finish with no_purchase/replenish_later; a previously confirmed routine remains the single displayed routine.
- Discovery long-description and plan-purpose goldens now describe sequential single recommendations in all three locales. Prior golden JSON is preserved in release evidence. DISC-MCP-07 compares the currently published contract rather than the obsolete literal 9.0.0; the checksum remains pinned to the published artifact. DISC-DET-04 expects the explicit single-recommendation discovery identity.

## Explicit cost-weight discovery (2026-09-10)
