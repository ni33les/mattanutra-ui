...
test('SPLAN-SPEC-01/02 six tools and the plan card teach flat conversation without private knowledge', () => {
  ...
  for (const word of ['Thailand', 'call execute directly', 'scoring', 'idempotencyKey', 'expectedRevision']) assert.ok(plan.description.includes(word), word);
...
test('SPLAN-SPEC-03 only the current contract is published and all examples validate', () => {
  ...
  for (const resource of CONTRACT_RESOURCES) assert.ok(resource.uri.includes('/11.1.0/'), resource.uri);
...
test('SPLAN-SPEC-02 instructions and guides describe the same bounded weight/reset protocol', () => {
  for (const locale of ['en', 'th', 'zh-CN']) {
    ...
    assert.ok(instructions.includes('scoring')); assert.ok(guide.includes('scoring')); assert.ok(guide.includes('call execute directly'));
