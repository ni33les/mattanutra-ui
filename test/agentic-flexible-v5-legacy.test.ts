...
import type { ExecuteSuccessWire, PlanSuccessWire, PublicErrorWire } from "../lib/agentic/contract/outputs.ts";
const nutrientNames = ["Alpha", "Beta", "Gamma", "Delta", "Epsilon", "Zeta", "Eta", "Theta"];
const supplements = nutrientNames.map((name, index) => {
    const uuid = `32345678-1234-1234-1234-${String(index + 1).padStart(12, "0")}`;
...
const legacyFixture = JSON.parse(fixtureBytes.toString()) as {
    ...
    request: PlanRequest;
    created: PlanSuccessWire;
    checkout: ExecuteSuccessWire;
    creationMutations: StoredMutation[];
    checkoutMutations: StoredMutation[];
};
...
type Replies = {
    plan: PlanSuccessWire;
    execute: ExecuteSuccessWire;
    order: { ok: true; totalPriceMinor: number; paymentStatus: string };
};
...
async function legacyPlan(explicit: boolean, retainOriginal = true) {
    ...
    const persistedHistorical = await runtime.store.getPlanRevision(planId, created.revision);
    assert.ok(persistedHistorical);
    return { runtime, created, planId, historical: persistedHistorical, original };
}
for (const paid of [false, true])
    test(`LEGACY5-05 frozen ${paid ? "paid" : "unpaid"} checkout survives v4 marking and catalogue replacement`, async () => {
        ...
        const checkout = legacyFixture.checkout;
        assert.equal(checkout.ok, true);
        if (!checkout.ok)
            throw new Error(checkout.error.message);
        assert.deepEqual(checkout, legacyFixture.checkout);
        if (paid)
            await simulatePayment({ config: runtime.config, scope: runtime.scope, store: runtime.store, now: legacyFixture.now, orderHandle: checkout.orderHandle, scenario: "success" });
        const before = await call(runtime, "order", { orderHandle: checkout.orderHandle });
        ...
        const retired = await call(runtime, "execute", payload);
    assert.equal(retired.ok, false); if (retired.ok) throw new Error("Retired plans cannot execute");
    assert.equal(retired.error.reasonCode, "not_found");
        const after = await call(runtime, "order", { orderHandle: checkout.orderHandle });
        ...
        const capturedOrder = legacyFixture.checkoutMutations.find(item => item.method === "insertOrder")!.arguments[0] as { id: string; frozenPlan: unknown };
    const order = await runtime.store.getOrder(capturedOrder.id); assert.ok(order);
    const orders = [order];
    assert.equal((orders[0]!.frozenPlan as { items: unknown[] }).items.length, 6);
    assert.deepEqual(orders[0]!.frozenPlan, (legacyFixture.checkoutMutations.find(item => item.method === "insertOrder")!.arguments[0] as { frozenPlan: unknown }).frozenPlan);
    });
