...
const { currentWebCheckoutRecommendations, findReusableWebCheckoutPayment } = checkout;
...
function database(runRevision: string | null, currentRevision = '7') {
  ...
  const sql = async (parts: TemplateStringsArray) => {
    ...
    if (query.includes('product_recommendation_runs')) return [{ id: runId, catalogue_revision: runRevision, selection_revision: 0, input_revision: 1, current_selection_revision: 0, excluded_product_ids: [],
      diagnostics: { matching: { selectedOptionId: 'option', options: [{ optionId: 'option', productIds, recommendations: [] }] } } }];
  ...
  return { calls, sql: sql as unknown as Parameters<typeof currentWebCheckoutRecommendations>[0] };
}
const selection = { planId, locale: 'en' as const, selectedItemIds: productIds, recommendationRunId: runId, optionId: 'option', assessmentRevision: 1, selectionRevision: 0 };
it('V5-CHECKOUT-01: changed or unproven catalogue provenance rejects new checkout before reading purchase lines', async () => {
  for (const revision of ['6', null]) {
    ...
    await assert.rejects(currentWebCheckoutRecommendations(sql, selection), (error: unknown) => error instanceof Error && 'code' in error && error.code === 'stale_product_selection');
...
it('V5-CHECKOUT-02: current provenance accepts all eight selected products and keeps their quote rows', async () => {
  ...
  const rows = await currentWebCheckoutRecommendations(sql, selection);
...
it('V5-CHECKOUT-03: exact existing intent resumes its frozen payment without catalogue or price reads', async () => {
  ...
  assert.equal(await findReusableWebCheckoutPayment(sql, { ...input, optionId: 'changed' }), null);
