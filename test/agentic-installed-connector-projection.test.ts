...
describe("actual installed connector projection evidence", () => {
  const published = { contractVersion: "6.0.0", schemaChecksum: "sha", tools: ["info", "plan", "execute", "order", "support", "feedback", "evidence"].map(name => ({ name, description: `${name} current`, inputSchema: { type: "object", properties: { operation: { enum: ["create", "get", "revise", "answer", "select"] } } } })) };
  ...
  it("ANNA-AX-06 preserves local schema checks while refusing to mistake native discovery for an installed projection", () => {
    ...
    assert.equal(validateInstalledConnectorProjection({ ...evidence, tools: evidence.tools.slice(0, 6) }, published).passed, false);
  });
