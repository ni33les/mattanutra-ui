...
function basketNames(plan: Record<string, unknown>) {
    ...
    const choices = plan.choices as Array<{ products: Array<{ name: string }> }>;
    assert.ok(choices.length);
    return choices.flatMap(choice => choice.products.map(item => item.name));
}
function d3Fixture() {
    const found = fixtureSnapshot().products.find((item) => item.candidate.title === "Vitamin D3 2000 IU");
    assert.ok(found);
    return found;
}
function cloneProduct(source: CatalogueProduct, patch: {
    orderable?: boolean;
...
describe("MCP sale states", () => {
    it("lists the public tools and advertises flat plan fields", async () => {
        ...
        assert.match(encoded, /"scoring"/);
        assert.match(encoded, /"excludeSupplementIds"/);
        assert.match(encoded, /"female"/);
        assert.match(encoded, /"male"/);
        assert.match(String(plan.description ?? ""), /unknown.*zero/i);
        const branches = schema.anyOf as Array<{
            properties?: Record<string, Record<string, unknown>>;
        }>;
        const create = branches.find(branch => Boolean(branch.properties?.targets && !branch.properties?.planHandle));
        assert.ok(create, "create has a flat schema branch");
        ...
        assert.ok(Object.keys(requestProperties).length > 0);
        {
            ...
            assert.ok(requestProperties.scoring);
            assert.ok(requestProperties.requirements);
            const sex = (requestProperties.profile.properties as Record<string, {
                enum?: string[];
            }>).sex;
    ...
    it("may use an already-approved selected SKU whose validation failed", async () => {
        ...
        replaceCatalogueSnapshot({
            ...base,
            products: base.products.map((item) => item.candidate.title === "Vitamin D3 2000 IU"
                ? {
                    ...item,
                    candidate: {
                        ...item.candidate,
        ...
        const runtime = runtimeFor();
        const created = structured(await call(runtime, "plan", D3_PLAN));
        assert.equal(basketNames(created).some((name) => name === "Vitamin D3 2000 IU"), true);
    });
});
