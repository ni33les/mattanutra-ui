...
export async function paidFixture(sql: NonNullable<ReturnType<typeof getSql>>, { plan = true, adoption = true, receipt = true, accounting = true } = {}) {
  ...
  if (receipt) await sql`insert into bpm(id,ray,event_name,event_type,event_status,emitted_by,plan_id,selected_plan,properties)
    values (${randomUUID()}::uuid,${randomUUID()},'payment_fulfillment_succeeded','payment','paid','stripe_payment_flow',${planId}::uuid,'precision',${sql.json({ paymentId: id })})`;
  return p;
}
