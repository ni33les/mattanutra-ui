...
it("V5-INFRA-05 complete matcher pack uses real isolated HTTP handlers without a browser build", { timeout: 45_000 }, async () => {
  ...
  const env = { ...isolatedValidationEnvironment(process.env), AGENTIC_BUILD_ID: (existsSync(".next/required-server-files.json") ? JSON.parse(readFileSync(".next/required-server-files.json", "utf8")).config.env.AGENTIC_BUILD_ID : process.env.AGENTIC_BUILD_ID), NODE_OPTIONS: "--max-old-space-size=768" };
