...
async function create(dailyServings: number) {
  ...
  const plan = await call(runtime, "plan", { operation: "create", idempotencyKey: `pack-quote-create-${dailyServings}`,
    request: { destinationCountry: "TH", locale: "en", optimization: "lowest_cost", profile: { ageYears: 35, lifeStage: "adult" },
...
for (const dailyServings of [1, 2, 3]) it(`quotes one purchased bottle consistently at ${dailyServings} daily servings`, async () => {
  ...
  assert.equal(record(plan.stackSummary).totalPriceMinor, 48500);
  assert.equal(plan.estimatedOrderTotalMinor, 53500);
  ...
  const publicFrozen = record(order.frozenOrder);
...
it("uses purchased lines when a saved v4 aggregate is stale, then preserves the frozen checkout on unpaid and paid replay", async () => {
  ...
  assert.equal(record(order.frozenOrder).totalPriceMinor, 53500);
});
