...
test("LOCK-RESOURCE-01 UAT-equivalent evidence rejects unlimited CPU, excess memory, swap and OOM",async()=>{
  const module=new URL("../../scripts/service-efficiency/runtime-resources.mjs",import.meta.url);
  assert.ok(existsSync(module),"Resource equivalence must be measured and enforced");
  const {validateUatResources,scopedBenchmarkCommand}=await import(module.href);
