...
Release target: DEV, then UAT for the matching-lock package. PRD is unchanged. Evidence is stored outside the checkout. The independent
...
revision publication and order/payment transitions retain short invariant fences.
Catalogue fences remain only for mutations validating current commercial facts;
matching publication stores immutable snapshots without that catalogue lock. Task creation joins the caller's transaction; wake
...
available. Restart application supervisors and their worker children together,
verifying the compiled source identity and actual execution in each environment.
