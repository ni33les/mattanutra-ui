...
test("AXR-REL-03 polling reports failed refinement instead of returning the previous ready revision", async () => {
  ...
  const polled = await rpc(instance, "plan", { operation: "get", planHandle: initial.planHandle });
  assert.equal(polled.ok, false, "A failed revision 2 must never look like a successful revision 1");
  assert.equal(polled.error.reasonCode, "stale_revision"); assert.equal(polled.error.currentRevision, 1);
  assert.equal(polled.error.requestedRevision, 2);
