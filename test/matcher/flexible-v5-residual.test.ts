...
it('V5-SEARCH-05: a companion product creates a supported remaining-gap quantity absent from standalone breakpoints', () => {
  ...
  assert.deepEqual(result.selected?.variantIds.slice().sort(), ['seller:companion:x1', 'seller:small:x6']);
  assert.deepEqual(result.selected?.variantDoses?.find(row => row.productId === 'small'), { productId: 'small', dailyUnits: 6, dailyPills: 6 });
});
