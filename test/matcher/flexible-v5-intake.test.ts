...
it('V5-INTAKE-01: supported midpoint quantities balance estimated intake without claiming it as known coverage', () => {
  ...
  const selected = closestDoseOption(result);
    assert.deepEqual(selected?.variantIds, ['seller:small:x5']);
    // 20..80 existing + 50 new = 70..130: either endpoint deviates 30/100.
    assert.equal(selected?.doseFit?.total, 0.3);
    assert.equal(selected?.coverageSummary?.[0]?.knownCurrent, 0);
    assert.equal(selected?.coverageSummary?.[0]?.estimatedCurrent, 50);
    assert.equal(selected?.coverageSummary?.[0]?.newContribution, 50);
    assert.equal(selected?.coverageSummary?.[0]?.remainingGap, 50);
    assert.equal(selected?.coveredCount, 0);
  }
});
it('V5-INTAKE-02: estimated range breakpoints also account for products already fixed in the basket', () => {
  ...
  const selected = closestDoseOption(result);
  assert.ok(selected?.variantIds.includes('seller:small:x4'));
  // A is 10..30 + 40 fixed + 40 new = 90..110; B is exactly 100.
  assert.equal(selected?.doseFit?.total, 0.1);
  assert.equal(selected?.coverageSummary?.find(row => row.subjectId === 'a')?.knownTotal, 80);
});
it('V5-INTAKE-03: midpoint candidates do not change the additional safety-limit excess penalty', () => {
  ...
  const result = match(r, catalog([product('small', { a: 10 })]));
  const selected = closestDoseOption(result);
  assert.deepEqual(selected?.variantIds, ['seller:small:x4']);
  // Four units gives 60..120 (loss 0.4); five gives 70..130 with
  // worst-case loss 30/100 + 2*(10/120), so the safer fit wins.
  assert.equal(selected?.doseFit?.total, 0.4);
  assert.equal(selected?.doseFit?.limitWeight, 2);
