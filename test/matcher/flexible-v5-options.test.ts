...
it('V5-OPTION-02: the price profile recommends an almost-exact cheap choice and retains exact fit', () => {
  ...
  assert.deepEqual(closestDoseOption(result).productIds, ['exact']);
  assert.deepEqual(result.selected?.productIds, ['cheap']);
  const cheaper = result.selected;
...
it('V5-OPTION-03: a simpler partial choice remains explicit and option roles are deduplicated', () => {
  ...
  assert.equal(closestDoseOption(result).productCount, 2);
