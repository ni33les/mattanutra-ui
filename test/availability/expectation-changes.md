...
- AVAIL-COPY-01–04: raw positive coverage remains positive; exact zero and unknown have distinct localized labels. The strictly-greater-than-12% headline rule is unchanged. The existing empty-diagnostics assertion now requires null, not invented zero; unknown rows do not manufacture food gaps. The production reveal label formatter covers exact-zero, tiny-positive, missing and pending observations in three locales. Browser count cases preserve their immutable fixtures; the attempted direct UPDATE was rejected by the append-only guard and is not retained.
