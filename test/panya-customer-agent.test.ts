...
describe("Nong Mata customer agent architecture", () => {
  ...
  it("keeps the Nong Mata admin page platform-only and dashboard-backed", () => {
    ...
    assert.match(dashboard, /showDashboardTimeframeControls[\s\S]*!\(view === "panya" && panyaSection === "configuration"\)/);
    ...
    assert.match(panyaView, /Modality controls/);
    assert.match(panyaView, /Welcome brief/);
    assert.match(panyaView, /welcomeBriefLivingProtocol/);
    assert.match(panyaView, /welcomeBriefRightAmountFormula/);
    assert.match(panyaView, /welcomeBriefUnpaid/);
  ...
  it("uses governed tool policy and recurring check-ins without sending raw text to BPM", () => {
    ...
    assert.match(panya, /buildPanyaWelcomeContext/);
    assert.match(panya, /panyaWelcomeGenerationInput/);
    assert.match(panya, /messageType: "panya_welcome"/);
    assert.match(panya, /updateCommunicationMessageStatus/);
    assert.match(panya, /panya_welcome_archived/);
