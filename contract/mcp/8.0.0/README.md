# MattaNutra client interaction guide 8.0.0
...
Flow: info → agree name/amount/unit/basis → plan(create) → explain best_match and closest_dose, highlightedAlternativeOptionId and advice → revise/answer/select → confirm with the person → execute → poll order.
...
Conversation returns advice flags for the selected and highlighted options, plus plan-wide flags: adviceId, guidanceIds, optionIds, kind and severity only. It does not include advice messages, labels or evidence bodies. Request details.sections=["advice"] with the relevant optionIds for those complete findings; plan-wide findings are in details.safetyGuidance. Repeated incomplete-information flags are combined without changing the stored findings. A product_data flag is not a measured health-limit breach. Null supplyDays stays unknown. Read dose_review and interaction details before explaining those findings. No advice flag requires an acknowledgement or prevents purchase.
...
Review and mutation examples use responseView=conversation; polling examples use status and requested-information examples use details. Omitting responseView on a plan returns conversation, except for the temporary legacy header described above. Order omission continues to return its full view; use the explicit order examples. Mutation inputs permit conversation or full; presentation does not affect the business idempotency key. Conversation is a short decision: option cards contain optionId, roles, reason, stackSummary and coveragePercent. They retain the original IDs and ordering; selectedOptionId and highlightedAlternativeOptionId point into those cards. Cards do not contain basket lines or per-target coverage. Advice flags contain identifiers, kind and severity only. Other returned options remain selectable. Fetch products, coverage or advice in one details batch for the relevant optionIds when the customer needs them, including the exact products and quantities before confirming checkout. Full/detail views retain complete labels, findings, arithmetic and schedules.

| Information | Conversation | Details |
| --- | --- | --- |
| `doseFit` | Not returned | `sections=["score"]` |
| `claimIds` | Not returned | `sections=["advice"]` |
| Labels and administration | Not returned | `sections=["products"]` |
| Advice bodies and sources | IDs, kind and severity only | `sections=["advice"]` |
| Per-target coverage | Percentage only | `sections=["coverage"]` |
...
| “That option will do.” | select its returned optionId with current expectedRevision, read its products/coverage and relevant advice details, then confirm before execute |
...
```json
{
  ...
  "params": {
    ...
    "arguments": {
      "operation": "revise",
      "idempotencyKey": "example-importance-v8-01",
      "planHandle": "cap_replace_with_plan_handle_from_create",
      "expectedRevision": 1,
      "requestPatch": {
        "requirements": {
