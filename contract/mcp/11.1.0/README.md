...
Call plan with flat targets/context and idempotencyKey to create. Wait for the current tool call to finish. SSE plan calls wait up to 15 seconds for committed matching results. Poll only if its final result says processing, using planHandle and pollAfterSeconds. JSON-only calls return immediately. Retry a lost mutation response with the same key and input. Disconnecting does not cancel admitted work. Handle-only calls read existing work; no matching occurs in polls.
