...
A plan returns one recommendation. Confirmation uses only `planHandle`, `expectedRevision` and a new `idempotencyKey`; it advances the revision without matching again. Public option identifiers are removed from plan, feedback and checkout schemas, examples and responses. A handle-only read remains read-only; `scoring:{}` remains refinement/recovery. Internal candidate bookkeeping and private frozen payment encodings do not become new protocol fields.
