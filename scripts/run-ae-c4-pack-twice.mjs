...
if (!process.execArgv.includes(strip)) {
  ...
  const { canonicalAeC4Report, runAeC4Pack } = await import(
    "../test/agentic-ae-c4-pack.test.ts"
  );
  const a = await runAeC4Pack();
  const b = await runAeC4Pack();
  const left = canonicalAeC4Report(a);
  const right = canonicalAeC4Report(b);
