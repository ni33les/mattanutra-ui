...
if (!process.execArgv.includes(strip)) {
  ...
  const { canonicalAeC6Report, runAeC6Pack } = await import(
    "../test/agentic-ae-c6-pack.test.ts"
  );
  const a = await runAeC6Pack();
  const b = await runAeC6Pack();
  const left = canonicalAeC6Report(a);
  const right = canonicalAeC6Report(b);
