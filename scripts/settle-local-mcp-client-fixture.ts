...
const endpoint = new URL(process.env.MCP_URL ?? "http://127.0.0.1:3100/api/mcp");
assert.equal(endpoint.protocol, "http:"); assert.equal(endpoint.hostname, "127.0.0.1");
assert.ok(endpoint.port && !["80", "443", "3000", "5432"].includes(endpoint.port));
assert.equal(endpoint.pathname, "/api/mcp"); assert.equal(endpoint.search + endpoint.hash + endpoint.username + endpoint.password, "");
assert.equal(receipt.endpoint, endpoint.href, "The receipt must belong to this exact isolated HTTP candidate");
