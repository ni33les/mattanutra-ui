...
for (const database of (packageId === "practical" && mode === "validate" ? [] : [false, true])) {
  ...
  batches.push(await runBatch(label, ["--test", "--test-concurrency=1", "--experimental-test-module-mocks", "--experimental-strip-types", ...(packageId === "pharmacy-line" ? ["--loader", "./scripts/matcher-http-loader.mjs"] : packageId === "web-matching" ? ["--loader", "./test/payment-return/next-loader.mjs"] : []), ...(!database || ["payment-replay", "pharmacy", "pharmacy-source", "pharmacy-reveal", "pharmacy-line"].includes(packageId) ? ["--import", "./test/helpers/offline-network.mjs"] : []), "--import", "./scripts/register-ts-path-loader.mjs", ...selected], env, output));
...
async function prepareCompiledBuild() {
  ...
  await command("production-build", ["node_modules/next/dist/bin/next", "build", "--webpack"], { ...safe, NODE_ENV: "production", NEXT_BUILD_SKIP_TYPECHECK: "1", ...(["efficiency", "pharmacy-reveal", "pharmacy-animation", "pharmacy-line"].includes(packageId) ? { NODE_OPTIONS: "--max-old-space-size=4096", NEXT_BUILD_CPUS: "1" } : {}) });
}
if (mode === "validate") {
  ...
  if (["efficiency", "practical", "availability", "pharmacy", "pharmacy-landing", "pharmacy-reveal", "pharmacy-source", "pharmacy-animation", "pharmacy-line"].includes(packageId)) {
