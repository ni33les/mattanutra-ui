...
export async function benchmarkServices(output, env, inventory) {
  ...
  const report = { version: 1, passed: true, reproducible: true, controlCommit: releaseBase, candidateCommit: git(process.cwd(), "rev-parse", "HEAD"),
    ...
    measurements: "Process CPU/RSS includes worker threads. IPC bytes are V8-equivalent measurement bytes; checkpoint bytes are actual encoded payload bytes. Instrumentation serialization time is reported separately. SQL bytes count returned JSON rows. No clinical input or fixture changes.",
  ...
  const rows = runs.flatMap(run => run.comparison.rows.map(row => [run.run, row.id, row.control.wallMs, row.candidate.wallMs,
    ...
    row.control.warm?.rowBytes ?? "", row.candidate.warm?.rowBytes ?? ""]));
  const headers = ["run", "workload", "control_ms", "candidate_ms", "control_cpu_ms", "candidate_cpu_ms", "control_peak_rss", "candidate_peak_rss", "control_ipc_bytes", "candidate_ipc_bytes", "control_checkpoint_bytes", "candidate_checkpoint_bytes", "control_warm_db_bytes", "candidate_warm_db_bytes"];
