...
try {
  await sql`
    create table if not exists public.product_identifiers (
      ...
      constraint product_identifiers_type_check check (
        identifier_type in ('ean13', 'upc', 'manufacturer_sku', 'retailer_local_code', 'supplier_code')
      ),
      ...
      constraint product_identifiers_upc_check check (
        identifier_type <> 'upc' or normalized_value ~ '^[0-9]{12}$'
      )
    )
  `;
  ...
  await sql`
    create table if not exists public.product_identifier_candidates (
      ...
      constraint product_identifier_candidates_upc_check check (
        identifier_type <> 'upc' or normalized_value ~ '^[0-9]{12}$'
      )
    )
  `;
  ...
  await sql`
    alter table public.product_identifiers
    drop constraint if exists product_identifiers_upc_check
  `;
  ...
  await sql`
    alter table public.product_identifiers
    add constraint product_identifiers_upc_check check (
      identifier_type <> 'upc' or normalized_value ~ '^[0-9]{12}$'
    )
  `;
  ...
  await sql`
    alter table public.product_identifier_candidates
    drop constraint if exists product_identifier_candidates_upc_check
  `;
  ...
  await sql`
    alter table public.product_identifier_candidates
    add constraint product_identifier_candidates_upc_check check (
      identifier_type <> 'upc' or normalized_value ~ '^[0-9]{12}$'
    )
  `;
