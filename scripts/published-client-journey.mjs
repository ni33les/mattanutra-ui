...
export async function runConversationalJourney({ rpc, locale = "en", discovery = "tools_only", key = "documented-client", wait = ms => new Promise(done => setTimeout(done, ms)), checkout = false }) {
  ...
  if (discovery !== "schema_only") {
    ...
    assert.ok(info.clientExamples.some(row => row.name === "create-checkout"));
    if (discovery === "resources") {
      ...
      assert.ok(text.includes("call execute directly") && text.includes("scoring"));
    }
  }
  ...
  assert.equal(plan.nextAction, "execute"); assert.equal(plan.choices.length, 1); assert.equal(plan.scoring.profile, "best_match");
  const original = plan;
  ...
  const stale = await call("plan", { planHandle: plan.planHandle, expectedRevision: original.revision, idempotencyKey: `${key}-stale`, scoring: {} });
