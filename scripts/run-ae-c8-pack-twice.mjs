...
if (!process.execArgv.includes(strip)) {
  ...
  const { canonicalAeC8Report, runAeC8Pack } = await import(
    "../test/agentic-ae-c8-pack.test.ts"
  );
  const a = await runAeC8Pack();
  const b = await runAeC8Pack();
  const left = canonicalAeC8Report(a);
  const right = canonicalAeC8Report(b);
