...
import { getEvidenceBundle } from "@/lib/agentic/qa/control-plane";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

type RouteProps = Readonly<{
  params: Promise<{ evidenceBundleHandle: string }>;
}>;
...
export async function GET(request: Request, { params }: RouteProps) {
  ...
  const { evidenceBundleHandle } = await params;
  const result = getEvidenceBundle(evidenceBundleHandle);
