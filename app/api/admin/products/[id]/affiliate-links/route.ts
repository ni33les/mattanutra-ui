...
import {
  ...
  upsertProductAffiliateLink
} from "@/lib/admin-products";
import { isUuid } from "@/lib/assessment-store";

export const runtime = "nodejs";

type ProductAffiliateLinksRouteProps = Readonly<{
  params: Promise<{
    id: string;
  }>;
}>;
...
export async function POST(
  request: Request,
  { params }: ProductAffiliateLinksRouteProps
) {
  ...
  try {
    const row = await upsertProductAffiliateLink({
    ...
    console.error("Unable to add product affiliate link", error);

    return NextResponse.json(
      { message: "Unable to add product affiliate link" },
      {
        headers: {
          "Cache-Control": "no-store"
        },
