...
function agentDescriptionKey(row: AdminAgentRow) {
  if (row.name === "Product Matcher") return "productMatcher";
  if (row.name === "Nutrition Plan Formulator") return "nutritionPlanFormulator";
  if (row.name === "HealthScore Engine") return "healthScoreEngine";
  if (row.name === "Nutrition Plan Advisor") return "nutritionPlanAdvisor";
  if (row.name === "Communications Coordinator") return "communicationsCoordinator";
  if (row.name === "Email Dispatcher") return "emailDispatcher";
  if (row.name === "Content Publisher") return "contentPublisher";
  if (row.name === "Scheduler") return "scheduler";
  if (row.type === "human") return "human";
...
function agentDescription(row: AdminAgentRow, locale: Locale) {
  return t(
    locale,
    `admin.agents.descriptions.${agentDescriptionKey(row)}` as MessageId
  );
}
