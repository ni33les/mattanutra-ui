...
export function PharmacyResults({ locale, sourceLocale = locale, slug, pharmacyName, planId, revision, deep = false, initialResult = null, receipt = null }: {
  locale: Locale; sourceLocale?: Locale; slug: string; pharmacyName: string; planId: string; revision: number; deep?: boolean;
  ...
  }, [deep, healthAttempt, planId, revision, sourceLocale, slug, receipt]);
  const text = (value: LocalizedText | undefined) => value ? getLocalizedText(value, locale) : "";
  if (!result) return <section className="mx-auto max-w-3xl px-6 py-20 text-center" aria-live="polite">
    <h1 className="font-serif text-4xl">{c.preparing}</h1><p className="mt-5">{polling.failed ? c.error : c.preparingBody}</p>
    {polling.failed && <button onClick={polling.retry} className="mt-6 rounded-xl bg-[var(--mn-teal-deep)] px-6 py-3 text-white">{c.retry}</button>}
  </section>;
  ...
  return <div className="mx-auto w-full max-w-6xl px-6 py-12 sm:px-8">
    ...
    <h1 className="mt-3 font-serif text-4xl sm:text-5xl">{deep ? c.details : c.title}</h1>
    ...
    </> : <PharmacyOrderPanel key={result.productRecommendations?.runId ?? revision} planId={planId} slug={slug} locale={locale}
      revision={revision} pharmacyName={pharmacyName} initialReceipt={receipt} onRefresh={polling.retry} />}
  </div>;
}
