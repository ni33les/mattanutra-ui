...
- `LOCK-SNAPSHOT-04` reproduces and prevents Node async-context imports in the shared browser matcher. The asynchronous scope is installed by a server-only module; a real Webpack browser bundle must compile and execute the same reference reader. Concurrent server scopes and durable restart tests remain unchanged in meaning.
