...
| Admin catalogue simulations still inject six; shipping inserts show at most four products | Simulate unrestricted matching; retain every ordered product and its instructions, paginating the customer PDF. Web options publish null when pill counts are unknown. | `matcher-v5-product-surfaces`, maintained retail insert wiring assertions |
...
| Matcher telemetry stores coverage in an integer column and loses fractional events with PostgreSQL 22P02 | Widen the column to unrestricted numeric without deleting historical events; preserve fractional and unknown coverage, verify the deployed type, and run the migration before acceptance fingerprints. | `agentic-fractional-telemetry.integration` actual writer and legacy migration/replay cases |
...
| Legacy ready/empty results invite confirmation while reporting purchase ineligible | Resolve one effective operational status for public and compact responses, summary and actions; retain reviewable purchase alternatives and explicit future-replenishment semantics. | `agentic-operational-decision` nonempty advice, empty recommendation and horizon regressions |
| A low budget or empty child-dose recommendation requires a compulsory answer | Preserve customer ceilings through unrelated revisions, change/clear only explicitly, and keep the child's 110mg reference with serious advice on the selectable 300mg trade-off. | Maintained budget-recovery case in `agentic-p1` and child-reference case in `agentic-phase0-child-ul` |
| AX2/AX3 allow only v4 option fields and hardcode v4 discovery | Retain compact output checks while admitting the published v5 roles, purchase eligibility and coverage summary; lock current schema to the reviewed v5 artifact. Pack versions advance to 2.2/3.2. | Existing AX2-08, AX2-13, AX3-10 and schema-lock consumers |
| Empty default uses generic no-purchase timing despite selectable alternatives | Summary, concise explanation and next action explicitly invite reviewing purchasable trade-offs; selection remains required. | EN/TH/ZH `agentic-flexible-v5-journey` review-options cases |
| Untrusted labels can leak into incidental or total exposure | Use the canonical verified coverage ledger; keep raw label quantities and confidence separate, and mark total exposure incomplete. | `matcher/flexible-v5-provenance`, `agentic-flexible-v5-evidence-trust` |
...
| Catalogue identity omits commercial completeness, orderability, approval or nutrient aliases | Hash complete product and nutrient-resolution inputs with stable ordering; changed eligibility cannot reuse an earlier purchase option. | `agentic-catalogue-cache-identity`, maintained R2-PACK-05 quarantine/recovery |
| A closest-dose choice can inherit a lowest-cost reason despite unknown horizon economics | Label the canonical closest-dose role consistently in top-level and option reasons; retain null future costs and duration uncertainty. | EN/TH/ZH `agentic-flexible-v5-journey` current-intake cases; R4-DUR-01/03 |
| Repeated dose variants repeat identical safety fact resolution | Resolve facts once within the immutable request/product lifetime; preserve per-fact rational rounding. Standard and expanded complete results remain byte-identical. | `matcher/flexible-v5-safety-compilation`; four complete-result comparisons in external performance evidence |
| Customer-value replay compares only assertion pass counts | Retain complete normalized request/response business evidence and explicitly compare both runs; generated identities, documented clocks and latency alone are normalized. | `agentic-cv-evidence-hygiene`, maintained IMPL/R2/R3 repeat-run equality |
...
| DET pin probe uses a memory store without the live freeze's catalogue epoch | Validate publication against the exact captured epoch inside fixture transactions; stale epochs still fail. DET scoring expectations are unchanged. | `agentic-det-snapshot-store` create/select and stale-epoch regressions |
