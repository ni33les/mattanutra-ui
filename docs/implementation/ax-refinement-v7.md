# Reliable refinement work package

Source baseline: `6baeab0e`; deployed control: `22bce180`. DEV only; UAT unchanged.

## Decisions

Linear closest-dose arithmetic and 2× safety excess stay unchanged. Numerical preferences remain advisory. Search improvements use 8,000/64,000 attempts. Experimental scoring stays offline.

Only the reviewed work-package inventory and affected regressions run. This user-authorized scope replaces AXR-REG-03 and the full-suite portions of AXR-HYG-01/02; no full-suite green claim.

## Evidence

Immutable execution evidence: `/root/.codex/visualizations/2026/09/08/ax-refinement-v7`.
The original six-profile response report/content-hashed catalogue is unavailable locally. Requests are preserved verbatim from the supplied brief; existing corrected DEV/UAT reconstructions retain separate identities. Historical reliability remains NOT_REPRODUCED until a meaningful pre-fix execution reproduces its boundary.

## Slice ledger

1. Baseline and scoped runner implemented. Historical requests and scoped exclusions are explicit.
2. Durable operations, atomic task admission, leases, cancellation fencing and deadline handoff implemented; operation and PostgreSQL behavioural cases passed.
3. Serializable bounded cursor, sparse complementary patterns, actual replacement opportunities and equivalent seller quotes implemented. The corrected A4/A5 controls pass at 8,000 attempts. Expanded A2 recovery remains under the original worker deadlines.
4. Existing-option highlighting, zero-coverage wording and continued-supply no-purchase completion implemented; 18 initial published client profiles passed in English, Thai and Chinese. Quantity proposal and clearing checks were then added.
5. Seven identifiable attachment products audited. Two supported DEV-only corrections reconstruct in isolation; unsupported packs and unavailable original union remain explicitly unresolved.
6. Advice kinds, confirmed lower bounds, explicit algae source semantics and generated v7 contract implemented. Historical contracts retained.
7. Explicit scoped validator and DEV-only attested deployment path implemented. Paired acceptance and live rollout remain pending.

## Expectation changes

Record each intentional old/new expectation with its AXR ID before updating existing assertions. Historical v4–v6 schemas, catalogue prices and evidence remain unchanged.

### Reliability evidence

`02-handoff-red-2.log` exposes missing handoff against pre-fix business code using real reconstructed data and a controlled barrier. `02-worker-sequence.log` completes A2 revisions 1–4 through actual matcher workers. `02-operations-green.log` and `02-postgres-operations-2.log` cover duplicate ownership, cancellation fencing and atomic queue admission. The first PostgreSQL attempt passed assertions but hung during cleanup; it remains incomplete, and cleanup was corrected before rerunning. Original UAT request IDs were not found in the local journal.

### Intentional expectation changes recorded during implementation

- `test/agentic-phase4-algae.test.ts` and the source-name assertions in `test/matcher/phase2.test.ts`: v7 preserves an explicit `fish_allowed` or omitted source preference instead of deriving `algae_only` from wording or storing it because the diet is vegan. The exact algae alias is resolved only with an explicit `algae_only`; otherwise the unresolved target has a nonblocking clarification. E-02 keeps its exact algae SKU assertion with an explicit algae source input. Prices and unrelated assertions are unchanged. Replacement cases: AXR-TERM-01–02.
- `AXR-ALT-01–03` add a pointer into the existing option set; the presentation tests compare the original supplied arrays after projection. No basket mutation is permitted in that slice.
- `AXR-NOP-01–02` distinguish confirmed continued-intake completion from an empty dose-fit recommendation with unresolved gaps. Historical future-schedule cases without such coverage evidence retain their prior expectations.
- `AXR-ADV-01–03` add rule classification and a confirmed pill lower bound without changing thresholds, evidence, exact unknown totals, selection eligibility or acknowledgement requirements.

Scoped evidence remains in `/root/.codex/visualizations/2026/09/08/ax-refinement-v7`. Intermediate timeouts, interrupted runs and RED outputs are retained. No whole-application or whole-MCP green claim has been made.

### Comparison harness correction

The first comparison attempts (linear-comparison-1/2) are invalid evidence: the canonical request captured an empty reference list before worker setup installed the frozen references. The worker cannot repair an explicit request-level list. The harness now asserts exact frozen reference content before either implementation runs. The preliminary price-regression assertion was replaced with the correctly referenced control (loss 0.25 + 0.5 + 0.5 + 2 × 4.3/350; first-order goods 53,700 minor THB). Fixture prices were not changed. Both invalid reports and RED attempts remain preserved; they are not acceptance evidence.

### Focused consumer execution

The second affected-consumer attempt executed 135 assertions successfully, including existing checkout/payment replay and the PostgreSQL web selection fixture. Its overall source attestation is deliberately false because implementation continued during the focused run. Final acceptance must run unchanged source twice. The first consumer attempt was interrupted after a memory-only suite opened an unnecessary ambient database pool; memory and PostgreSQL tests now receive separate environments.

The A2 sequence now uses the real three-second handoff and polls returned handles. Each create/revise operation retains the existing 90-second client bound and 15-second worker bound. The earlier 90-second aggregate timeout combined four requests, including two 64,000-attempt searches, and is retained as failed evidence; it is replaced by bounded subcases for each request, not by a longer request or worker deadline.

### Inventory reconciliation before release

The first run of acceptance attempt 2 passed 209 cases in 28 files. A release-diff review identified two changed existing test files absent from that inventory: the historical contract compatibility suite and matcher phase-2 source/oversupply suite. The incomplete paired attempt is retained. Both files are now explicitly scoped, and the runner rejects any changed work-package test omitted from the inventory (the separately completed offline experiment remains the control). The new detection case failed before implementation; its focused run with both affected files passed all 17 assertions. Final paired acceptance uses the corrected inventory.

### Browser boundary caught by the production build

Acceptance attempt 3 passed 223 cases twice with identical complete journey transcripts, the linear control comparison, typecheck and release-diff lint. Its first build was killed by host memory pressure. The build-only recovery then exposed Node-only checkpoint/hash/equality imports reachable from the admin browser matcher. No attestation or deployment was issued for that source. The initial new import-graph probe had a directory-resolution defect; its corrected pre-fix execution explicitly reproduces all six forbidden Node imports. The shared cursor now uses the existing browser-safe SHA-256 utility and portable exact fact equality, while V8/compression codecs are imported only by server matching. The corrected boundary and worker checkpoint tests pass; final acceptance includes this additional directly affected browser boundary.

### DEV expanded smoke and recovery correction

Acceptance 4 passed 225 scoped cases twice at 83960547, with identical journey semantics, comparison, typecheck, release lint and build. DEV deployment and the six ordinary live journeys plus mock checkout recovery passed. The expanded live operation then failed: a large JSONB checkpoint write hit the existing statement timeout at 28,000 attempts; recovery encountered a changed observation timestamp, and polling incorrectly returned the previous committed revision. This is a newly reproduced DEV failure, not proof of the historical UAT A2 cause. `08-live-expanded-failure.json` and both smoke attempts remain preserved.

The corrective RED commit reproduces inline checkpoint rewriting, clock-only identity mismatch and failed-revision polling. Compressed archives now use an additive EXTERNAL bytea column; lifecycle metadata updates preserve those bytes, legacy JSONB archives migrate lazily, and lease/cancellation fencing remains intact. Content-identical observation-clock refreshes resume; actual product/reference changes still invalidate work. Failed polls identify the requested and last committed revisions. The matcher identity advances to flexible-dose-fit-6 without changing scoring.

`09-expanded-postgres-2.log` passes the actual expanded worker after an injected 28,000-attempt checkpoint failure: the saved 24,000-attempt cursor plus 4,000 reserved attempts resume to 64,000 total, with the previous revision unchanged until publication. The first integration setup attempt had inconsistent fixture/reference epochs and is invalid evidence. The clock test compares decoded cursor semantics and full outputs with only its deliberately varied availabilityAsOf normalized; V8 byte-for-byte output is not a semantic identity guarantee. No public score, dose, advice or price field was removed from assertions. Final unchanged-source acceptance and corrected DEV rollout follow.
