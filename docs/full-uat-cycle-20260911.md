...
`acceptance-4` completed 2,902 application, 164 PostgreSQL, 2,126 MCP replay, 120 PostgreSQL replay and 43 browser cases with no functional failures; 2,490 replay events matched. All twelve documented client/payment journeys and their paired semantics passed. Its final attestation still failed: the standalone report switched the dose-check corpus, discarded immutable reference identity, expected nonexistent matcher report fields and leaked the documented-client catalogue between passes. `MCP-REPORT-02–05` now reproduce those defects before correction. The offline dose corpus is consistently the captured 154-listing snapshot, value fixtures retain complete reference identities, matcher scores retain the existing nine-point bar, and documented clients bind their declared catalogue through the existing immutable execution context. Historical catalogue prices and score arithmetic are unchanged.

The same acceptance audit found a test-owned `lock_review_task_results` table left after teardown. Exact reconstruction of the preflight schema proved this was the only schema difference; catalogue fingerprints were identical. `FULL-CYCLE-11` fails in teardown until the suite drops its fixture table. The final gate continues to hash the entire schema rather than excluding test-created objects. The raw RED hook failure is retained even though this Node version reported exit zero; execution-proof reconciliation rejects failed suite hooks independently of the process exit status.

The full `acceptance-5` cycle passed and source `96c06512` was deployed to DEV and UAT. A live UAT cold D3 operation nevertheless missed the 15-second readiness condition: its first public ready response arrived at 16.55 seconds. The worker claimed promptly (0.28 seconds), and the operation completed in 15.33 seconds. A second replica's first match overlapped web matching and also missed the public observation bound; a warmed run passed. Both failures remain preserved and are not replaced by the warmed pass.

`FULL-CYCLE-12–13` were committed RED in `416564bd` before adding bounded matching-thread preparation. Matching-capable external workers now load the execution modules and prepare their existing two threads before registering capacity or listening for wake-ups. Preparation uses no request, catalogue, database access or search attempts, and the existing CPU/thread bounds remain unchanged. Actual calculations, scores, checkpoints and deadlines are unchanged. The closed-pool assertion preserves the pool's existing `MatcherUnavailableError` rather than inventing a new error string. The new unchanged-source acceptance and cold deployment checks must pass before this follow-up is signed off.
